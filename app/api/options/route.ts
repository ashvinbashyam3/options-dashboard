...
type MassiveUnderlyingSnapshot = {
...
const pickUnderlyingPrice = (snapshot?: MassiveUnderlyingSnapshot | null): number | null => {
  if (!snapshot) {
    return null;
  }
  const priceFromTrade = parseNumber(snapshot.last_trade?.price);
  if (priceFromTrade !== null) {
    return priceFromTrade;
  }
  const priceFromClose = parseNumber(snapshot.day?.close);
  if (priceFromClose !== null) {
    return priceFromClose;
  }
  const priceFromBid = parseNumber(snapshot.last_quote?.bid);
  if (priceFromBid !== null) {
    return priceFromBid;
  }
  const priceFromAsk = parseNumber(snapshot.last_quote?.ask);
  return priceFromAsk;
};
...
export async function GET(request: NextRequest) {
  ...
  for (let page = 0; page < MAX_PAGES && nextUrl; page += 1) {
    ...
    pageRows.forEach((row) => {
      ...
      if (underlyingPrice === null) {
        const candidate = pickUnderlyingPrice(row.underlying_asset);
        if (candidate !== null) {
