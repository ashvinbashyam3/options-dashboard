...
export async function GET(request: NextRequest) {
  ...
  for (let page = 0; page < MAX_PAGES && nextUrl; page += 1) {
    ...
    if (json.underlying_asset) {
      console.log(
        `[api/options] Received top-level underlying snapshot keys: ${Object.keys(json.underlying_asset)}`
      );
    } else {
      console.log("[api/options] No top-level underlying snapshot present on this page");
    }
    ...
    if (underlyingPrice === null) {
      ...
      if (snapshotCandidate !== null) {
        ...
        console.log("[api/options] Top-level snapshot missing numeric price fields");
      }
    }
    ...
    pageRows.forEach((row) => {
      ...
      if (underlyingPrice === null) {
        ...
        } else if (row.underlying_asset) {
          console.log(
            "[api/options] Row contained underlying_asset but no numeric price:",
  ...
  if (underlyingPrice === null && rows.length > 0) {
    console.warn(
      "[api/options] Unable to derive underlying price; sample underlying asset:",
      rows[0]?.underlying_asset
    );
  }
