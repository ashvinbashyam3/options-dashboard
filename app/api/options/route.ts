...
const fetchUnderlyingFallback = async (
  ...
  const fallbackUrl = new URL(`${MASSIVE_STOCK_SNAPSHOT_BASE}${encodeURIComponent(ticker)}`);
  fallbackUrl.searchParams.set("limit", "1");
  ensureApiKey(fallbackUrl, apiKey);
  console.warn(
    `[api/options] Attempting stock snapshot fallback for ${ticker}: ${fallbackUrl.toString()}`
  );
  const response = await fetch(fallbackUrl.toString(), { headers, cache: "no-store" });
  if (!response.ok) {
    const text = await response.text();
    console.error(
      `[api/options] Stock snapshot fallback failed with ${response.status}: ${text}`
    );
    return null;
  }
  ...
  console.log(
    `[api/options] Stock snapshot fallback returned ${snapshots.length} entries for ${ticker}`
  );
  for (const snapshot of snapshots) {
    const candidate = pickUnderlyingPrice(snapshot);
    if (candidate !== null) {
      console.log(
