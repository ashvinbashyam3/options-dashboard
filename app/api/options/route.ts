...
const extractSnapshotsFromPayload = (payload: unknown): MassiveUnderlyingSnapshot[] => {
  ...
  if (typeof payload === "object") {
    const objectPayload = payload as { results?: unknown; underlying_asset?: unknown };
...
const fetchUnderlyingFallback = async (
  ...
  for (const attempt of stockSnapshotAttempts) {
    ...
    logger.warn(`Attempting stock snapshot fallback (${attempt.description}): ${url}`);
    const response = await fetch(url.toString(), { headers, cache: "no-store" });
    if (!response.ok) {
      const text = await response.text();
      logger.error(
        `Stock snapshot fallback (${attempt.description}) failed with ${response.status}: ${text}`
      );
      continue;
    }
    let json: unknown;
    try {
      ...
      logger.error(
        `Stock snapshot fallback (${attempt.description}) returned invalid JSON: ${String(err)}`
      );
      continue;
    }
    const snapshots = extractSnapshotsFromPayload(json);
    logger.info(
      `Stock snapshot fallback (${attempt.description}) returned ${snapshots.length} entries`
    );
    for (const snapshot of snapshots) {
      const candidate = pickUnderlyingPrice(snapshot, `fallback:${attempt.description}`, logger);
      if (candidate !== null) {
        logger.info(`Stock snapshot fallback detected underlying price ${candidate}`);
        return candidate;
      }
    }
  }
  logger.warn(`All stock snapshot fallback attempts failed for ${ticker}`);
  return null;
};
...
export async function GET(request: NextRequest) {
  ...
  if (underlyingPrice === null) {
    logger.warn(
      `No underlying price detected after ${rows.length} rows for ${ticker}; invoking fallback`
    );
    underlyingPrice = await fetchUnderlyingFallback(ticker, apiKey, authHeaders, logger);
  }
