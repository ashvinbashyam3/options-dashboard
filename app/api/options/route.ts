...
type MassiveStockSnapshotResponse = {
  results?: MassiveUnderlyingSnapshot[];
};

const MAX_PAGES = 15;
const MASSIVE_BASE = "https://api.massive.com/v3/snapshot/options/";
const MASSIVE_STOCK_BASE = "https://api.massive.com/v3/snapshot/stocks/";
...
const pickUnderlyingPrice = (
  ...
  if ("last_price" in snapshot) {
    candidates.push((snapshot as MassiveDaySnapshot).last_price);
  }
  if ("price" in snapshot) {
    candidates.push((snapshot as MassiveDaySnapshot).price);
  }
...
const pickUnderlyingFromRow = (row: MassiveOptionRow): number | null => {
  return pickUnderlyingPrice(row.underlying_asset);
};
...
export async function GET(request: NextRequest) {
  ...
  const fetchSpotFromStockSnapshot = async (): Promise<number | null> => {
    const stockUrl = new URL(`${MASSIVE_STOCK_BASE}${encodeURIComponent(ticker)}`);
    ensureApiKey(stockUrl, apiKey);
    console.log(`[api/options] Fallback stock snapshot lookup: ${stockUrl}`);
    const response = await fetch(stockUrl.toString(), { headers: authHeaders, cache: "no-store" });
    if (!response.ok) {
      const text = await response.text();
      console.warn(`(api/options) Stock snapshot request failed: ${response.status} ${text}`);
      return null;
    }
    const json = (await response.json()) as MassiveStockSnapshotResponse;
    const stockRows = Array.isArray(json.results) ? json.results : [];
    for (const entry of stockRows) {
      const candidate = pickUnderlyingPrice(entry);
      if (candidate !== null) {
        console.log(`[api/options] Stock snapshot provided spot: ${candidate}`);
        return candidate;
      }
      console.warn("[api/options] Stock snapshot entry missing numeric price", entry);
    }
    console.warn("[api/options] Stock snapshot returned no usable price");
    return null;
  };
  ...
  if (underlyingPrice === null) {
    console.warn(
      `[api/options] No underlying price detected after ${rows.length} rows for ${ticker}; attempting stock snapshot fallback.`
    );
    underlyingPrice = await fetchSpotFromStockSnapshot();
    if (underlyingPrice === null) {
      console.warn("[api/options] Stock snapshot fallback also failed to provide underlying price");
    }
  }
