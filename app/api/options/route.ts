...
const parseNumber = (value: unknown): number | null => {
  ...
  if (typeof value === "object") {
    const maybeNumericKeys = ["amount", "value", "price", "last", "close"];
    for (const key of maybeNumericKeys) {
      if (key in (value as Record<string, unknown>)) {
        const parsed = parseNumber((value as Record<string, unknown>)[key]);
        if (parsed !== null) {
          return parsed;
        }
      }
    ...
    for (const candidate of candidates) {
      ...
      if (parsed !== null) {
        if (typeof candidate === "object" && candidate !== null) {
          console.log("[api/options] Parsed nested underlying price candidate", candidate);
        }
        return parsed;
      }
    }
