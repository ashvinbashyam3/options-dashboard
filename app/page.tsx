...
export default function HomePage() {
  ...
  const fetchChain = useCallback(async (ticker: string) => {
    ...
    try {
      ...
      console.log(
        `[OptionsUI] Underlying price from API: ${
          payload.underlyingPrice !== null ? payload.underlyingPrice : "<null>"
        }`
      );
      if (payload.debugLogs?.length) {
        payload.debugLogs.forEach((entry) => {
          console.log(`[OptionsUI][api-debug] ${entry}`);
        });
      }
